...
#이미지 전처리
def preprocess(img):
    return (img / 255. - 0.5)*2       #-1~1사이값으로 스케일링
def deprocess(img):
    return (img + 1)/2   #0~1사이 값으로(원래 값으로 복원)
...
# fit, axes = plt.subplots(1, len(test_faces)+1, figsize=(20,16))
...
src_img = img1_faces[0]  #화장 안한게 source image
ref_img = img2_faces[0]  #화장 한게 reference image

X_img = preprocess(src_img)
X_img = np.expand_dims(X_img, axis=0)     #reshape

Y_img = preprocess(ref_img)
Y_img = np.expand_dims(Y_img, axis=0)

output = sess.run(Xs, feed_dict={X:X_img, Y:Y_img})   #feed_dict={입력, 출력}
